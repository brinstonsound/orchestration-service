...
function loadActionList () {
  ...
  if (fs.existsSync(actionsFolder)) {
    ...
    files.forEach(file => {
      lstActions.push(
        JSON.parse(fs.readFileSync(path.resolve(actionsFolder, file)))
      );
    });
  }
}
...
module.exports.createAction = async options => {
  try {
    ...
    fs.writeFileSync(
      path.resolve(actionsFolder, `${newId.toString()}.json`),
...
module.exports.updateAction = async options => {
  try {
    ...
    if (theAction.data.id != undefined) {
      ...
      // Update the file
      fs.writeFileSync(
        path.resolve(actionsFolder, `${options.id.toString()}.json`),
        JSON.stringify(theAction, null, 2)
      );
